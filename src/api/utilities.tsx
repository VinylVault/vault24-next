 ...
 */
export async function calculateReleaseDecade(
  releaseYear: number,
): Promise<string[]> {
 ...
 */
export async function sanitizeNamesTitles(
  nameTitle: string,
...
export async function calculateDate(addedDate: string, releaseDate: string) {
  ...
  const newAddition =
    process.env.IS_NEW_ADDITION &&
    (new Date().getTime() - new Date(addedDate).getTime()) / (3600 * 24) <=
      parseInt(process.env.IS_NEW_ADDITION);
  const newRelease =
    process.env.IS_NEW_RELEASE &&
    (new Date().getTime() - new Date(releaseDate).getTime()) / (3600 * 24) <=
      parseInt(process.env.IS_NEW_RELEASE);
