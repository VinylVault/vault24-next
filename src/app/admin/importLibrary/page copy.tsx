...
import { fetchReleaseDetails, releaseSlugs, writeArtists, writeFormats, writeGenres, writeRecordLabels, writeRelease, writeStyles } from "../../api/localConnect";
...
export default async function main() {
    ...
    for (let oneDiscogsRelease of fullDiscogsReleaseList) {
        ...
        let thisReleaseSlug = await generateSlugs(releaseTitle+" "+releaseId)
        // console.log(isReleaseInLibrary)
        
        console.log(thisReleaseSlug)
        console.log(releaseSlugList)

        if (releaseSlugList.includes(thisReleaseSlug)) {
            ...
            if (oneDiscogsRelease.basic_information.artists){
                for (let releaseArtist of oneDiscogsRelease.basic_information.artists) {
                    let releaseArtistSlug = await writeArtists(releaseArtist, 'library')
            ...
            if (oneDiscogsRelease.basic_information.labels){
                for (let releaseRecordLabel of oneDiscogsRelease.basic_information.labels) {
                    let releaseRecordLabelSlug = await writeRecordLabels(releaseRecordLabel, 'library')
            ...
            releaseSlug = await writeRelease(discogsReleaseDetail.data, oneDiscogsRelease.folder_id, oneDiscogsRelease.date_added, 'library', releaseGenreArray, releaseStyleArray, releaseFormatArray, releaseArtistArray, releaseRecordLabelArray, releaseShelfArray)
