...
export function ShowArchiveGridItems({
    ...
    return(
        ...
        <div className="flex flex-col items-middle gap-2 w-full shadow-lg shadow-vault-menubar hover:shadow-vault-link transition duration-500 rounded-xl">
            <Link href={`/show_archive/${blogPlaylistSlug}`} className="relative aspect-video w-full rounded-xl overflow-hidden ">
                <Image 
            ...
            <div className="flex flex-col items-middle">
                {blogPlaylistCurrent && <div className="bg-vault-menubar text-vault-live text-md font-bold text-center px-1 rounded-xl">CURRENT PLAYLIST</div>}
                <p className="text-vault-title mx-2 mb-1 font-bold text-4xl italic">{blogPlaylistTitle}</p>
                <p className="text-vault-genre mx-2 mb-1 text-2xl italic">{blogPlaylistIntro}</p>
                ...
                <p className="text-vault-text mx-2 mb-2 font-vault-title font-title text-xl">Published: {blogPlaylistPublishDate.toUTCString()}</p>
