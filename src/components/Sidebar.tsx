...
export function Sidebar() {
    return (
        <>
        <aside className="sticky top-0 overflow-y-auto scrollbar-hidden pb-4 flex-col flex ml-1 pl-4 center lg:hidden">
            ...
            <SmallSidebarItem Icon={Twitter} title="Twitter" url="/social/twitter" />
            <SmallSidebarItem Icon={Facebook} title="Facebook" url="/social/facebook" />
            <SmallSidebarItem Icon={Twitch} title="Twitch" url="/social/twitch" />
            <SmallSidebarItem  Icon={Youtube} title="YouTube" url="/social/youtube" />
            <SmallSidebarItem Icon={Disc3} title="Discogs" url="/social/discogs" />
            <hr/>
            <SmallSidebarItem Icon={Coffee} title="Buy A Coffee" url="/buy-a-coffee" />
            <hr/>
            <SmallSidebarItem Icon={Cookie} title="Privacy Policy" url="/privacy-policy" />
            <SmallSidebarItem Icon={Hammer} title="GDPR Policy" url="/gdpr-policy" />
            <SmallSidebarItem Icon={LifeBuoy} title="Terms of Service" url="/terms-of-service" />
            <SmallSidebarItem Icon={UserCheck} title="Show Subsctiption Terms" url="/show-subscription-terms" />
        </aside>
        <aside className="lg:sticky top-0 w-56 absolute overflow-y-auto scrollbar-hidden pb-5 flex-col gap-2 px-2 flex">
...
function LargeSidebarItem({ isActive = false, Icon, title, url }: LargeSidebarItemProps) {
    return (
        <Link href={url} className={twMerge(buttonStyles({size: 'icon'}), `text-vault-link w-full flex items-center rounded-lg gap-4 p-3 ${ isActive ? "font-bold bg-vault-link text-vault-background" : undefined }`)}>
            <Icon className="w-6 h-6"/>
...
function LargeSidebarSection({ children, title, visibleCount=Number.POSITIVE_INFINITY }: LargeSidebarSectionProps) {
    ...
    return (
        <div>
            {title && <div className="text-lg mt-4 ml-4 text-vault-title font-title text-center">{title}</div>}
            {visibleChildren}
            {showExpandButton && 
                <Button variant="genres" className="w-full flex items-center rounded-lg gap-3 p-3">Show More</Button>
            }
        </div>
    )
}